...
export function SessionKeyExample() {
  ...
  const { tgId, scw, publicKey, verificationCode } = useParams();
  ...
  const sessionkeyClick = async () => {
    if (publicKey && publicKey.startsWith('0x')) {
      ...
      const bind = await botRequest({ url: `api/binding?tgId=${tgId}&scw=${scw}&serializeSessionKeyParams=${res}&verificationCode=${verificationCode}` })
